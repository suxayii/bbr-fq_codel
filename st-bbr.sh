...
# 支持 Debian / Ubuntu / CentOS / AlmaLinux / RockyLinux
#
# v4 (专业增强版):
# - 增加命令依赖预检测
# - 优化 sysctl 更新逻辑，避免重复写入
# - 增加公网 IP 获取的备用源
# - 强化 sysctl 应用失败和 BBR 模块加载检测
# - 增加日志记录功能
# =========================================================
...
for cmd in curl ip lscpu sysctl awk sed grep tee; do
  if ! command -v $cmd >/dev/null 2>&1; then
    echo "❌ 缺少关键命令 '$cmd'，脚本无法继续。请先安装。"
    exit 1
  fi
done

# ---------------- 参数设置 ----------------
QDISC=${1:-fq} # 默认使用 fq，可指定 fq_codel
...
add_sysctl_param() {
  local key=$1 value=$2
  # 检查 key 是否存在
  if grep -qE "^\s*#?\s*${key}\s*=" /etc/sysctl.conf; then
    # 获取当前值
    current_value=$(grep -E "^\s*${key}\s*=" /etc/sysctl.conf | tail -n1 | awk -F'=' '{print $2}' | xargs)
    # 仅当值不同时才修改
    if [[ "$current_value" != "$value" ]]; then
      sed -i "s|^\s*#?\s*${key}\s*=.*|${key} = ${value}|" /etc/sysctl.conf
      echo "  -> 更新参数: ${key} = ${value}"
    fi
  else
    # 不存在则追加
...
if [[ $kernel_major -lt 4 ]] || ([[ $kernel_major -eq 4 ]] && [[ $kernel_minor -lt 9 ]]); then
  echo "❌ 当前内核版本过低（$(uname -r)），BBR 需要内核版本 ≥ 4.9"
  exit 1
fi
...
echo "应用 sysctl 配置..."
if ! sysctl -p; then
  echo "⚠️ sysctl 配置应用失败，请检查 /etc/sysctl.conf 文件格式或权限。"
  exit 1
fi
...
echo "✅ 当前默认队列调度器: $current_qdisc"

if [[ "$current_cc" != "bbr" ]]; then
  echo "⚠️ BBR 未立即生效，尝试加载 BBR 模块..."
  if modprobe tcp_bbr 2>/dev/null; then
    echo "tcp_bbr" > /etc/modules-load.d/bbr.conf
    echo "✅ BBR 模块已加载并设为开机自启"
  else
    echo "⚠️ BBR 模块可能已内置或不被支持，请检查"
  fi
fi

# ---------------- BBR 模块加载验证 ----------------
if lsmod | grep -q "tcp_bbr"; then
  echo "✅ BBR 模块 (tcp_bbr) 已成功加载。"
else
  echo "⚠️ 未检测到 tcp_bbr 模块加载。可能已内置于内核，建议重启系统后再次检查。"
fi

# ---------------- 验证网卡队列调度器 ----------------
echo "==== 验证网卡队列调度器 ===="
if ! command -v tc >/dev/null 2>&1; then
  echo "⚠️ 未找到 'tc' 命令，跳过此项验证。请安装 'iproute2' 包。"
else
  ...
  if [[ -n "$default_iface" ]]; then
    ...
    if tc qdisc show dev "$default_iface" | grep -qE "fq|fq_codel"; then
      echo "✅ $QDISC 已成功应用于 $default_iface"
    else
      echo "⚠️ 未在 $default_iface 上检测到 $QDISC，请检查配置是否成功应用"
    fi
  else
    echo "⚠️ 无法识别默认网卡，跳过此项验证"
  fi
fi
...
if ! command -v iperf3 >/dev/null 2>&1; then
  ...
  if command -v apt-get >/dev/null 2>&1; then
    ...
    echo "❌ 未检测到可用包管理器，请手动安装 iperf3 后再试"
  fi
fi
...
if command -v iperf3 >/dev/null 2>&1; then
  ...
  server_pid=$!
  sleep 1
  iperf3 -c 127.0.0.1 -t 3 || echo "⚠️ 测速失败（提示：请检查防火墙是否阻止了 5201 端口）"
...
echo ""
echo "==== 🎉 全部完成 ===="
echo "BBR 已成功启用。如果遇到问题，建议重启服务器以确保所有网络参数完全生效。"
